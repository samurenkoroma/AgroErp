...
const FieldsMap = ({fields, onSelectField}: {
    ...
    return (
        <div ref={mapContainer} className="w-full h-full min-h-[500px] rounded-xl overflow-hidden"
             style={{background: '#1a1a2e'}}/>
    );
};

// ==================== CARD COMPONENT FOR OTHER TABS ====================

const UnitCard = ({unit, onClick}: { unit: ProductionUnit; onClick: () => void }) => {
    ...
    const getStatusText = () => {
        if (unit.metadata?.status === 'active') return 'Активен';
        if (unit.metadata?.status === 'maintenance') return 'Обслуживание';
        return 'Планируется';
    };

    const formatArea = () => {
        if (!unit.geometry?.area) return null;
        return `${unit.geometry.area} ${unit.geometry.areaUnit === 'ha' ? 'га' : 'м²'}`;
    };
    ...
    return (
        ...
        >
            <div className="flex items-start gap-3">
                <div className="text-3xl">{getUnitIcon(unit.type)}</div>
              ...
              {getStatusText()}
                    ...
                    <p className="text-sm text-gray-500 mt-0.5">{getUnitTypeName(unit.type)}</p>
                    {formatArea() && (
                        <p className="text-xs text-gray-400 mt-1">Площадь: {formatArea()}</p>
                    )}
                    <div className="flex flex-wrap gap-1 mt-2">
                        {unit.capabilities.slice(0, 4).map((cap) => (
                            <span key={cap} className="text-xs text-gray-400" title={getCapabilityName(cap)}>
                {getCapabilityIcon(cap)}
              </span>
                        ))}
                        {unit.capabilities.length > 4 && (
                            <span className="text-xs text-gray-400">+{unit.capabilities.length - 4}</span>
                        )}
                    </div>
                </div>
            </div>
...
const UnitList = ({units, title, icon: Icon, onSelectUnit}: {
...
type TabType = 'fields' | 'greenhouses' | 'containers' | 'hydroponic' | 'plot';

const ProductionUnitsPage2 = () => {
    ...
    const tabs = [
        ...
        {id: 'plot', label: 'Участки', icon: Droplets, count: hydroponic.length},
            ...
            <div className="max-w-7xl mx-auto px-6 py-6">
                ...
                `}
                            >
                                ...
                                {tab.label}
                                <span
                                    className={`text-xs px-1.5 py-0.5 rounded-full ${isActive ? 'bg-green-200 dark:bg-green-800' : 'bg-gray-200 dark:bg-gray-700'}`}>
                  {tab.count}
                </span>
                            </button>
                        );
...
export default ProductionUnitsPage2;